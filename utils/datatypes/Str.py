...
class _Match :
    ...
    # and numeric backreferences (\1, \2) and named backreferences (\g<1>, \g<name>) are replaced by the contents of the corresponding group.
    # (?P<name>...)
    ...
    # This is identical to m.group(group). This allows easier access to an individual group from a match.
    ...
    def only_one_group(self, *, default = _no_value) :
        ...
        if len(_) > 1    : raise RuntimeError(f'{self} 中出现不止一个 group')
        elif len(_) == 0 :
            if default == self._no_value : raise RuntimeError(f'{self} 中不存在 group')
...
class Str(str) :
    ...
    # Return str(self).
    # def __str__(self) : return f'{type(self).__name__}\'{str.__str__(self)}\''
    
    # Required code that reproduces object
    # Return repr(self).
    # def __repr__(self) : return f'{type(self).__name__}({str.__repr__(self)})'
    ...
    def ensure_empty(self) :
        ...
        else               : raise RuntimeError(f'{self = }应该为空串')
    ...
    def find_all_match_iter(self, pattern, /, *, flags = 0) : return Iter(_Match(match) for match in re.finditer(pattern, self, flags))
    ...
    # Unknown escapes of ASCII letters are reserved for future use and treated as errors. Other unknown escapes such as \& are left alone.
    ...
    # Empty matches for the pattern are replaced only when not adjacent to a previous empty match, so sub('x*', '-', 'abxd') returns '-a-b--d-'.
    ...
    # \g<number> uses the corresponding group number; \g<2> is therefore equivalent to \2, but isn’t ambiguous in a replacement such as \g<2>0.
    ...
    def split(self, sep_or_pattern, /, *, maxsplit: int = None, reverse = False, re_mode = False, flags = 0) :
        ...
        if re_mode :
            ...
            if reverse                                        : raise RuntimeError(f'无法通过正则[{sep_or_pattern}]反向切割字符串[{self}]')
            if '(' in sep_or_pattern or ')' in sep_or_pattern : raise RuntimeError(f'暂不支持使用带括号的正则[{sep_or_pattern}]切割字符串[{self}]')
    ...
    def strip(self, string: str = ' \t\n', /, *, left: bool = True, right: bool = True) :
        ...
        else                      : raise ValueError(f'非法 {left=} 和 {right=}')
    ...
    def ensure_int(self) :
        ...
        else            : raise RuntimeError(f'[{self=}]不是整数')
    ...
    def ensure_float(self) :
        ...
        else              : raise RuntimeError(f'[{self=}]不是浮点数')
    ...
    # Return S left-justified or right-justified in a Unicode string of length width.
    # Padding is done using the specified fill character (default is a space).
    # NOT IN PLACE
    ...
    # i.e. upper- and titlecase characters may only follow uncased characters and lowercase characters only cased ones.
