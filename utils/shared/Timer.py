...
class Timer :
    ...
    def print_timing(cls, msg = '', *, delta: Optional[float] = None, indent: int = 0, color = None) :
        ...
        if delta is None     : print(Y(f'[{DateTime():%m-%d %H:%M:%S}] {cls._global_total:5.2f}s 间隔{timing_delta:9,.6f}s'), f'[ {msg} ]', flush = True)
        else                 : indent = '\t' * indent; print(Y(f'{indent}[{DateTime():%m-%d %H:%M:%S}] {cls._global_total:5.2f}s 本轮{delta:9,.6f}s'), f'[ {msg} ]', flush = True)
        cls._global_current = time.time()
        return cls
