...
def stat(word_form_list) :
    existing = []
    not_existing = []
    for raw_word_form in word_form_list :
        word_form = raw_word_form
        word_form = word_form.replace('/', ' or ')
        word_form = word_form.replace(' ', '%20')
        if exists(safe(path % word_form)) :
            if getsize(safe(path % word_form)) == 0 :
                remove(safe(path % word_form))
...
url = 'http://helloword.cn/api/a/c/r/wbe/wf'
...
for index, word_form in enumerate(not_existing) :
    open(safe(path % word_form), 'w').write(request(safe(url % word_form))['content'])
    print index, ctime(), safe(word_form), getsize(safe(path % word_form))
existing, not_existing = stat(word_form_list)
